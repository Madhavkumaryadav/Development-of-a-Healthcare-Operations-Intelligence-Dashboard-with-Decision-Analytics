...
Three workspaces:
  ...
  - Human Review: admin-only workspace, gated behind a login (checked against
    scrypt-hashed admin credentials — see src/admin_auth.py). Each uploaded PDF report here moves through
...
from src.report_store import answer_from_reports, build_report_payload, save_report_artifacts

# --- Admin credentials -------------------------------------------------
# Account metadata (name, department) lives in data/admin_credentials.csv —
# which has NO password column. Per-account salted scrypt hashes are stored
# in the git-ignored sidecar data/.admin_hashes.csv, generated by:
#     python -m src.admin_auth rotate
# Verification is done by src.admin_auth.authenticate(); nothing plaintext
# is ever stored or compared.
from src.admin_auth import authenticate as _authenticate_admin
...
st.markdown(
    ...
    <div class="upload-header">
        <div class="upload-header-content">
            <h1 class="upload-header-title">Upload & Custom Analysis</h1>
            <p class="upload-header-sub">Bring your own dataset — get live KPIs, charts, and a downloadable PDF report</p>
            <div class="upload-header-badge">📊 Custom Dataset · PDF Reports</div>
        </div>
    </div>
    """,
    unsafe_allow_html=True,
...
with tab_analysis:
    ...
    if not uploaded:
        ...
        if df_analysis.empty or df_analysis.shape[1] == 0:
            ...
            # Report scope controls are applied before charts, PDF, JSON, and RAG indexing.
            st.markdown("### Report scope")
            scope_cols = st.columns(3)
            with scope_cols[0]:
                date_range = None
                if datetime_col:
                    valid_dates = df_analysis[datetime_col].dropna()
                    if not valid_dates.empty:
                        date_range = st.date_input("Date range", value=(valid_dates.min().date(), valid_dates.max().date()), key="report_date_range")
            with scope_cols[1]:
                state_col = next((column for column in categorical_cols if "state" in column.lower()), None)
                selected_states = st.multiselect("States", sorted(df_analysis[state_col].dropna().astype(str).unique()) if state_col else [], key="report_states")
            with scope_cols[2]:
                subject_col = next((column for column in categorical_cols if any(word in column.lower() for word in ("disease", "program"))), None)
                selected_subjects = st.multiselect("Diseases / programs", sorted(df_analysis[subject_col].dropna().astype(str).unique()) if subject_col else [], key="report_subjects")

            report_df = df_analysis.copy()
            if datetime_col and date_range and len(date_range) == 2:
                report_df = report_df[report_df[datetime_col].dt.date.between(date_range[0], date_range[1])]
            if state_col and selected_states:
                report_df = report_df[report_df[state_col].astype(str).isin(selected_states)]
            if subject_col and selected_subjects:
                report_df = report_df[report_df[subject_col].astype(str).isin(selected_subjects)]
            if report_df.empty:
                st.warning("No rows match the selected report filters.")
                st.stop()
            df_analysis = report_df
            ...
            if categorical_cols and numeric_cols:
                ...
                if len(grp) >= 2:
                    fig_bar = go.Figure(go.Bar(
                        ...
                        customdata=[[int(v)] for v in grp["count"].fillna(0)],
                    ))
                    fig_bar.update_traces(
                        hovertemplate=(
                            "%{x}<br>Mean %{y:,.2f} ± %{error_y.array:.2f}"
            ...
            if datetime_col and numeric_cols:
                ...
                if len(ts_monthly) >= 2:
                    ...
                    fig_trend.update_traces(
                        hovertemplate="%{x|%b %Y}: <b>%{y:,.2f}</b> average<extra></extra>"
                    )
            ...
            option_cols = st.columns([2, 2, 3])
            with option_cols[0]:
                report_title = st.text_input("Report title", value="HealthSentinel Custom Analysis", key="report_title")
            with option_cols[1]:
                branding = st.text_input("Organisation / branding", key="report_branding")
            with option_cols[2]:
                report_sections = st.multiselect(
                    "Sections to include",
                    options=["overview", "insights", "numeric_summary", "distributions", "correlation", "category_comparison", "trend"],
                    default=["overview", "insights", "numeric_summary", "distributions", "correlation", "category_comparison", "trend"],
                    key="report_sections",
                )

            with st.container(border=True):
                st.markdown("#### Generate report")
                st.caption("Creates a PDF and matching JSON file from the selected rows and sections.")
                generate = st.button(
            ...
            if generate:
                with st.spinner("Building PDF report..."):
                    ...
                    payload = build_report_payload(
                        df=df_analysis, title=report_title, branding=branding,
                        filters={"date_range": [str(value) for value in date_range] if date_range else None,
                                 "states": selected_states, "subjects": selected_subjects},
                        sections=report_sections, numeric_cols=numeric_cols,
                        categorical_cols=categorical_cols, datetime_col=datetime_col,
                    ...
                    st.session_state["custom_analysis_pdf_bytes"] = build_pdf_report(
                        ...
                        report_title=report_title, branding=branding, sections=report_sections,
                    )
                    stem = f"analysis_report_{uploaded.name.rsplit('.', 1)[0]}_{payload['report_id']}"
                    _, json_path = save_report_artifacts(payload, st.session_state["custom_analysis_pdf_bytes"], stem)
                    st.session_state["custom_analysis_json_bytes"] = json_path.read_bytes()
                    st.session_state["custom_analysis_pdf_name"] = f"{stem}.pdf"
                    st.session_state["custom_analysis_json_name"] = f"{stem}.json"

            if st.session_state.get("custom_analysis_pdf_bytes"):
                download_cols = st.columns(2, gap="medium")
                ...
                if st.session_state.get("custom_analysis_json_bytes"):
                    with download_cols[1]:
                        st.download_button(
                            "⬇️ Download JSON",
                            data=st.session_state["custom_analysis_json_bytes"],
                            file_name=st.session_state.get("custom_analysis_json_name", "analysis_report.json"),
                            mime="application/json", use_container_width=True,
                            key="custom_analysis_json_download_btn",
                        )
